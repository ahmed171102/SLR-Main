"""
mobilenet_predictor.py — Arabic (ArSL) MobileNetV2 letter predictor.

Optional fallback that uses raw images.
"""
...
class ArabicMobileNetPredictor:
    """Predict Arabic letters from raw camera images using MobileNetV2.

    Args:
        model_path: Path to ``mobilenet_arabic_final.h5``.
    ...
    def __init__(
        ...
        logger.info(
            "ArabicMobileNetPredictor ready — %d classes, img=%dx%d",
            len(labels), image_size, image_size,
        )

    def predict(self, bgr_frame: np.ndarray) -> Optional[Tuple[str, float]]:
        """Predict Arabic letter from a BGR camera frame."""
